...
try:
    ...
    HAS_SKLEARN = False
    warnings.warn("scikit-learn not available. Some calibration features will be limited.")
...
class StatisticalCalibrator:
    ...
    def __init__(self,
        ...
        logger.info(f"Initialized StatisticalCalibrator with {n_folds} folds, "
                   f"{n_bootstrap} bootstrap samples")
    
    def calibrate_with_cv(self,
                         distances: np.ndarray,
                         labels: np.ndarray,
                         thresholds: Optional[np.ndarray] = None,
                         k_values: Optional[List[int]] = None,
                         metric: str = 'f1_score') -> pd.DataFrame:
        """
    ...
    def _evaluate_threshold_cv(self,
                              distances: np.ndarray,
                              labels: np.ndarray,
                              threshold: float,
        ...
        if HAS_SKLEARN and self.n_folds > 1:
            ...
            if min_class_samples >= self.n_folds:
                ...
                kfold = KFold(n_splits=min(self.n_folds, min_class_samples), shuffle=True)
    ...
    def _calculate_metrics(self,
        ...
        if HAS_SKLEARN:
            ...
            if len(np.unique(predictions)) == 1:
                metrics['precision'] = 1.0 if predictions[0] == 1 and labels[0] == 1 else 0.0
                metrics['recall'] = 1.0 if np.sum(labels) == 0 or (predictions[0] == 1 and np.all(labels == 1)) else 0.0
            ...
            if precision_recall_sum > epsilon:
                metrics['f1_score'] = 2 * (metrics['precision'] * metrics['recall']) / precision_recall_sum
            else:
                metrics['f1_score'] = 0.0
    ...
    def predict_with_threshold(self,
        ...
        if threshold is None:
            if self.optimal_threshold is None:
                raise ValueError("No optimal threshold available. Run calibration first.")
...
def ndcg_score(y_true: np.ndarray, y_scores: np.ndarray, k: Optional[int] = None) -> float:
    """
...
def create_calibration_report(calibrator: StatisticalCalibrator,
                             distances: np.ndarray,
    ...
    if calibrator.calibration_results is not None:
        ...
        for metric in ['f1_score', 'precision', 'recall', 'roc_auc']:
            if metric in df.columns:
                ...
                report.append(f"  {metric:12s}: threshold={best_row['threshold']:.4f}, "
        ...
        report.append(f"  Positive samples: {np.sum(labels)} ({100*np.mean(labels):.1f}%)")
        report.append(f"  Distance range: [{np.min(distances):.4f}, {np.max(distances):.4f}]")
...
if __name__ == "__main__":
    ...
    # Find optimal threshold
    optimal_threshold, optimal_f1 = calibrator.find_optimal_threshold(results, 'f1_score')
