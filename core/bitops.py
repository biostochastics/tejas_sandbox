...
# Import unified backend manager
try:
    from .backend_manager import get_backend_manager, ResourceContext

    HAS_BACKEND_MANAGER = True
except ImportError:
    HAS_BACKEND_MANAGER = False
    logger.info("UnifiedBackendManager not available, using legacy backend selection")
...
def pack_bits_rows(
    binary_matrix: Union[np.ndarray, "torch.Tensor"],
...
def unpack_bits_rows(
    ...
    # Unpack bits
    unpacked = np.unpackbits(packed_matrix, axis=1, bitorder=bitorder)
    # Trim to exact n_bits if needed
...
def hamming_distance_packed(
    ...
    if query_packed.shape[0] != database_packed.shape[1]:
        raise ValueError(
            f"Dimension mismatch: query has {query_packed.shape[0]} bytes, "
    ...
    if backend == "auto":
        ...
        if HAS_BACKEND_MANAGER:
            # Use unified backend manager for selection
            manager = get_backend_manager()
            backend = manager.select_backend(
                operation="hamming",
                data_size=n_items,
                data_type=database_packed.dtype,
                require_parallel=(n_items > 10000),
            )
            logger.debug(f"Backend manager selected: {backend}")

            # Map manager selection to our backends
            if "numba" in backend and HAS_NUMBA:
                backend = "numba"
            elif "native" in backend and sys.version_info >= (3, 10):
                backend = "native"
            else:
                backend = "numpy"
        else:
    ...
    # Route to appropriate implementation with performance tracking
    start_time = time.time()
    original_backend = backend
    fallback_triggered = False

    if backend == "numpy":
        distances = _hamming_distance_numpy(
            query_packed, database_packed, bitorder=bitorder
        )
    elif backend == "numba":
        if not HAS_NUMBA:
            # Performance warning for fallback
            warnings.warn(
                "Numba backend requested but not available. Falling back to numpy. "
                "This may cause 10-100x performance degradation for large datasets. "
            ...
            distances = _hamming_distance_numba(
                query_packed, database_packed, POPCOUNT_LUT
            )
    elif backend == "native":
        if sys.version_info < (3, 10):
            warnings.warn(
                f"Native backend requires Python 3.10+ for int.bit_count(). "
            ...
            distances = _hamming_distance_native(
                query_packed, database_packed, bitorder=bitorder
            )
    else:
        raise ValueError(
            f"Unknown backend: {backend}. Valid options: 'numpy', 'numba', 'native', 'auto'"
        )
    ...
    # Report performance to backend manager if available
    if HAS_BACKEND_MANAGER and not fallback_triggered:
        try:
            manager = get_backend_manager()
            manager.report_performance(
                backend=backend,
                operation="hamming",
                data_size=n_comparisons,
                elapsed_time=compute_time,
                success=True,
            )
        except Exception as e:
            logger.debug(f"Failed to report performance: {e}")

    # Log performance with fallback indication
    if fallback_triggered:
        logger.warning(
            f"Computed {n_comparisons} Hamming distances in {compute_time * 1000:.2f}ms "
            f"({throughput:,.0f} comparisons/sec) using FALLBACK {backend} backend "
            f"(originally requested: {original_backend})"
        )
    else:
        logger.debug(
            f"Computed {n_comparisons} Hamming distances in {compute_time * 1000:.2f}ms "
...
def _hamming_distance_numpy(
    query_packed: np.ndarray, database_packed: np.ndarray, bitorder: str = "little"
) -> np.ndarray:
    ...
    # Use np.bitwise_count if available (NumPy >= 1.20), otherwise fall back to LUT
...
def _hamming_distance_native(
    query_packed: np.ndarray, database_packed: np.ndarray, bitorder: str = "little"
) -> np.ndarray:
    """Pure native Python implementation using int.bit_count() for better performance.

    This version groups bytes into 64-bit integers for faster bit counting.
    Python 3.10+ required for int.bit_count() method.
    This implementation does NOT use numpy's bitwise operations to remain truly native.
    """
    n_items, n_bytes = database_packed.shape

    # Pure Python implementation - no numpy bitwise ops
    distances = np.zeros(n_items, dtype=np.int32)
    ...
    try:
        ...
        if n_bytes % bytes_per_chunk != 0:
            ...
            db_padded = np.pad(
                database_packed, ((0, 0), (0, pad_size)), constant_values=0
            )
        else:
...
def search_packed_vectorized(
    ...
    distances = hamming_distance_packed(
        query_packed, database_packed, backend=backend, bitorder=bitorder
    )
...
def validate_packing_roundtrip(
    binary_matrix: np.ndarray, bitorder: str = "little", n_bits: Optional[int] = None
) -> bool:
    """
    ...
    try:
        ...
        unpacked = unpack_bits_rows(
            packed, bitorder=bitorder, n_bits=n_bits or original.shape[1]
        )
...
def benchmark_backends(
    n_items: int = 50000, n_bits: Optional[Union[int, list]] = None
) -> dict:
    """
    ...
    for bits in n_bits:
        ...
        for backend in backends:
            try:
                # Warmup
                _ = hamming_distance_packed(
                    query_packed, database_packed[:100], backend=backend
                )
                ...
                for _ in range(3):
                    start_time = time.time()
                    distances = hamming_distance_packed(
                        query_packed, database_packed, backend=backend
                    )
                ...
                logger.info(
                    f"  {backend:>8}: {median_time * 1000:6.2f}ms, {throughput:>10,.0f} comp/sec"
                )
...
def print_benchmark_table(results: dict):
    ...
    for bit_config in bit_configs:
        ...
        for backend in backends:
            if backend in results[bit_config]:
                ...
                if "error" in res:
                    print(
                        f"{bits:<10} | {backend:<10} | {'ERROR':<12} | {res['error'][:40]}"
                    )
                else:
                    print(
                        f"{bits:<10} | {backend:<10} | {res['elapsed_ms']:<12.2f} | "
    ...
    for bit_config in bit_configs:
        for backend, res in results[bit_config].items():
            if (
                "throughput_per_sec" in res
                and res["throughput_per_sec"] > best_throughput
            ):
...
if __name__ == "__main__":
    ...
    print(
        f"   Memory reduction: {original_size} → {packed_size} bytes ({reduction:.1f}x)"
    )
