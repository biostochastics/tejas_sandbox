...
class TejasHeaderV2:
    ...
    def __init__(self, n_bits: int = 128, n_fingerprints: int = 0, 
                 bitorder: str = 'little', flags: int = 0b00000001):
...
def save_fingerprints_v2(filepath: Union[str, Path], 
                         fingerprints: np.ndarray,
                         titles: list,
                         metadata: Optional[Dict[str, Any]] = None,
                         bitorder: str = 'little') -> None:
    """
...
def load_fingerprints_v2(filepath: Union[str, Path]) -> Tuple[np.ndarray, list, Dict[str, Any]]:
    """
    ...
    if computed_checksum != header.checksum:
        logger.warning(f"Checksum mismatch: {computed_checksum:08x} != {header.checksum:08x}")
    ...
    fingerprints = np.frombuffer(fp_bytes, dtype=np.uint8).reshape(header.n_fingerprints, n_bytes_per_fp)
    offset += fp_size
    ...
    metadata.update(header.to_dict())
...
def validate_format_compatibility(fingerprints: np.ndarray, 
                                titles: list,
                                format_version: int = CURRENT_FORMAT_VERSION) -> bool:
    """
    ...
    try:
        if len(fingerprints) != len(titles):
            logger.error(f"Length mismatch: {len(fingerprints)} fingerprints != {len(titles)} titles")
            return False
        
        if format_version == FORMAT_VERSION_V2:
            # V2 requirements
            if fingerprints.dtype != np.uint8:
                logger.error(f"V2 requires uint8 fingerprints, got {fingerprints.dtype}")
                return False
            ...
            if fingerprints.shape[0] > 2**32 - 1:
                logger.error(f"Too many fingerprints for V2: {fingerprints.shape[0]} > {2**32-1}")
                return False
            
            n_bits = fingerprints.shape[1] * 8 if fingerprints.shape[1] < 32 else fingerprints.shape[1]
            if n_bits > 2**16 - 1:
