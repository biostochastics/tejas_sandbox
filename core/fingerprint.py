...
import hashlib
from core.bitops import unpack_bits_rows, search_packed_vectorized
...
class BinaryFingerprintSearch:
    ...
    def __init__(
        ...
        if enable_memory_pool:
            ...
            for size in common_sizes:
                if size <= len(self.titles):
                    self.memory_pool[f"distances_{size}"] = np.zeros(
                        size, dtype=np.int32
                    )
                    self.memory_pool[f"indices_{size}"] = np.zeros(size, dtype=np.int32)
            logger.info(f"Memory pool initialized with {len(self.memory_pool)} buffers")
        ...
        if self.format == "packed":
            ...
            logger.info(
                f"Detected packed format: {self.fingerprints_packed.shape} → {self.n_bits} bits"
            )
        else:
        ...
        logger.info(
            f"Format: {self.format}, Backend: {self.backend}, Device: {self.device}"
        )
    ...
    def _detect_format(self, fp: np.ndarray, hint: Optional[str] = None) -> str:
        ...
        if is_binary:
            ...
            if n_cols >= 32:  # Typical bit counts: 64, 128, 256
                ...
                if n_cols % 8 == 0 and n_cols <= 16:
                    detected = "packed"
                    logger.info(
                        f"Detected packed: binary values but only {n_cols} bytes"
                    )
                else:
                    detected = "unpacked"
                    logger.info(
                        "Detected unpacked: binary values, non-byte-aligned width"
                    )
            else:
                detected = "unpacked"
                logger.info("Detected unpacked: binary values, default")
        else:
            # Non-binary values - definitely packed
            if dtype == np.uint8 and n_cols < 32:
                detected = "packed"
                logger.info(
                    f"Detected packed: non-binary uint8 values (range=[{min_val},{max_val}])"
                )
            else:
                # Unusual case - non-binary with large width or non-uint8
                logger.warning(
                    f"Unusual format: non-binary values with dtype={dtype}, width={n_cols}"
                )
                logger.warning(
                    f"Value range=[{min_val},{max_val}], unique count={len(unique_vals)}"
                )
        ...
        if detected == "packed":
            ...
            expected_widths = [
                ...
                32,
                64,
            ]  # Common byte counts for 64, 128, 256, 512 bits
    ...
    def search(
        self,
        query_fingerprint: Union[torch.Tensor, np.ndarray],
        k: int = 10,
        ...
        if self.format == "unpacked":
            ...
            if query_format == "packed":
                query_tensor = unpack_fingerprints(
                    query_np.reshape(1, -1), n_bits=self.n_bits, bitorder=self.bitorder
                )[0]
            else:
                query_tensor = (
            ...
            if query_format == "unpacked":
                query_packed = pack_fingerprints(
                    query_np.reshape(1, -1), bitorder=self.bitorder
                )[0]
            else:
                query_packed = query_np
            distances, indices = self._search_packed(
                query_packed, k, backend=search_backend
            )
        ...
        for idx, sim, dist in zip(indices, similarities, distances):
            results.append((self.titles[int(idx)], float(sim), int(dist)))

        # Log performance
        comparisons_per_sec = (
            len(self.titles) / search_time if search_time > 0 else float("inf")
        )
    ...
    def _get_pooled_buffer(self, name: str, size: int, dtype: np.dtype) -> np.ndarray:
        """Get a buffer from the memory pool or allocate a new one."""
        if not self.enable_memory_pool:
            return np.zeros(size, dtype=dtype)

        pool_key = f"{name}_{size}"
        if pool_key in self.memory_pool:
            return self.memory_pool[pool_key]
        else:
            # Allocate and cache for future use
            buffer = np.zeros(size, dtype=dtype)
            # Only cache reasonable sizes
            if size <= 50000:
                self.memory_pool[pool_key] = buffer
            return buffer

    def _search_packed(
        self, query_np: np.ndarray, k: int, backend: Optional[str] = None
    ...
    def _search_unpacked(
        self, query_fingerprint: Union[torch.Tensor, np.ndarray], k: int
    ) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        if self.fingerprints is None:
            ...
            self.fingerprints = unpack_fingerprints(
                self.fingerprints_packed, bitorder=self.bitorder, n_bits=self.n_bits
            ).to(self.device)
        ...
        elif query_tensor.dim() > 2:
            query_tensor = query_tensor.view(-1)

        # Ensure query has same dtype as fingerprints
        if self.fingerprints.dtype != query_tensor.dtype:
            query_tensor = query_tensor.to(self.fingerprints.dtype)

        # Original XOR-based search
        xor_result = self.fingerprints ^ query_tensor.unsqueeze(0)
        hamming_distances = xor_result.sum(dim=1)

        # Get top-k nearest - always use stable sort for consistency
        # This ensures packed and unpacked formats produce identical results
    ...
    def search_pattern(
        self, pattern: str, encoder, max_results: int = 100
    ) -> List[Tuple[str, float, int]]:
        """
        ...
        # Report findings
        logger.info("Pattern Match Analysis:")
        ...
        if len(pattern_matches) + len(false_positives) > 0:
            logger.info(
                f"  Precision: {len(pattern_matches) / (len(pattern_matches) + len(false_positives)) * 100:.1f}%"
            )
    ...
    def _analyze_patterns(self, results: List[Tuple[str, float, int]]):
        ...
        if any(count > len(results) * 0.3 for count in patterns.values()):
            ...
            for pattern, count in sorted(
                ...
                if count > 0:
                    logger.info(
                        f"  {pattern}: {count}/{len(results)} ({count / len(results) * 100:.0f}%)"
                    )
    ...
    def benchmark(self, n_queries: int = 100):
        ...
        for idx in query_indices:
            ...
            if self.format == "unpacked":
                ...
                query = unpack_bits_rows(
                    packed_row, bitorder=self.bitorder, n_bits=self.n_bits
                )[0]
        ...
        logger.info(
            f"  Comparisons/sec: {len(self.titles) / search_times.mean() * 1000:,.0f}"
        )
...
def unpack_fingerprints(packed, n_bits: int = None, bitorder: str = "little"):
    ...
    if n_bits is None:
        ...
        unpacked_full = np.unpackbits(
            pk_np, axis=1, count=n_bits, bitorder=bitorder
        ).astype(np.uint8)
