...
# Feature flag for fast pairwise distance computation
import os

TEJAS_FAST_HAMMING = os.getenv("TEJAS_FAST_HAMMING", "0") == "1"


def fast_pairwise_distances(X, metric="hamming"):
    """
    Optimized pairwise distance with dimension-aware strategy.
    CRITICAL: Preserves original metric semantics.

    Args:
        X: Binary fingerprint matrix (n_samples, n_features)
        metric: Distance metric to use (only 'hamming' supported)

    Returns:
        distances: Pairwise distance matrix (n_samples, n_samples)
    """
    if metric != "hamming":
        raise ValueError(f"Unsupported metric: {metric}. Only 'hamming' is supported.")
    ...
    # Determine optimal block size based on array size
    if n_samples < 100:
        # Small arrays - compute directly without blocking
        # Expand dimensions for broadcasting
        X_i = X[:, None, :]  # (n_samples, 1, n_features)
        X_j = X[None, :, :]  # (1, n_samples, n_features)

        # XOR for binary difference
        xor_result = X_i ^ X_j  # (n_samples, n_samples, n_features)

        # Count differing bits
        if hasattr(np, "bitwise_count"):
            # Use bitwise_count if available (NumPy 2.0+)
            distances = np.bitwise_count(xor_result.astype(np.uint8)).sum(axis=2)
        elif hasattr(np, "bit_count"):
            # Use bit_count if available (NumPy 1.20+)
            distances = np.bit_count(xor_result.astype(np.uint8)).sum(axis=2)
        else:
            # Fallback: count bit differences manually
            distances = np.sum(xor_result != 0, axis=2)

        # Normalize by number of features to get proportion
        return distances.astype(np.float32) / n_features

    # Blocked computation for larger arrays
    block_size = min(500, n_samples)  # Adaptive block size
    ...
    for i in range(n_blocks):
        ...
        block_i = X[start_i:end_i]

        for j in range(n_blocks):
            start_j = j * block_size
            end_j = min(start_j + block_size, n_samples)
            block_j = X[start_j:end_j]

            # Efficient Hamming distance computation
            # Expand dimensions for broadcasting
            block_i_exp = block_i[:, None, :]  # (block_i_size, 1, n_features)
            block_j_exp = block_j[None, :, :]  # (1, block_j_size, n_features)

            # XOR for binary difference
            xor_result = block_i_exp != block_j_exp  # Works for any dtype

            # Count differing bits
            block_dist = np.sum(xor_result, axis=2)

            # Store in distance matrix
            distances[start_i:end_i, start_j:end_j] = block_dist.astype(np.float32)

    # Normalize by number of features to get proportion
    distances = distances / n_features

    return distances


# Constant for log base conversion
_LOG2 = np.log(2.0)


def safe_entropy(p):
    """
    Numerically stable binary entropy H(p) in bits.

    Computes H(p) = -(p*log2(p) + (1-p)*log2(1-p)) without NaN at boundaries.
    Uses scipy.special functions to handle 0*log(0) = 0 limit correctly.

    Args:
        p: Probability value(s) in range [0, 1]. Can be scalar or array.

    Returns:
        Binary entropy in bits. Same shape as input.
    """
    if not HAS_SCIPY or xlogy is None:
        # Fallback to clipping approach if scipy not available
        eps = 1e-10
    ...
    # Use xlogy for p*log(p) term - handles p=0 correctly
    # Use xlog1py for (1-p)*log(1-p) term - numerically stable near p=1
    # Convert from natural log to log2 by dividing by log(2)
    entropy = -(xlogy(p, p) + xlog1py(1.0 - p, -p)) / _LOG2
    return entropy
...
class DriftMonitor:
    ...
    def __init__(
        ...
        if baseline_fingerprints is not None and hasattr(
            baseline_fingerprints, "numpy"
        ):
        ...
        logger.info(
            f"  Baseline: {'Set' if baseline_fingerprints is not None else 'None'}"
        )
    ...
    def _compute_baseline_stats(self) -> None:
        ...
        bit_entropy = safe_entropy(activation_rates)
        ...
        if len(fp) > 1:
            if TEJAS_FAST_HAMMING:
                # Use optimized pairwise distance computation
                # Sample for efficiency if very large
                if len(fp) > 5000:
                    sample_idx = np.random.choice(len(fp), 5000, replace=False)
                    fp_sample = fp[sample_idx]
                else:
                    fp_sample = fp

                # Compute full pairwise distance matrix
                dist_matrix = fast_pairwise_distances(fp_sample, metric="hamming")
                # Extract upper triangle (excluding diagonal)
                upper_tri_indices = np.triu_indices(len(fp_sample), k=1)
                hamming_distances = (
                    dist_matrix[upper_tri_indices] * fp.shape[1]
                )  # Convert back to counts

                mean_hamming = (
                    np.mean(hamming_distances) if len(hamming_distances) > 0 else 0
                )
                std_hamming = (
                    np.std(hamming_distances) if len(hamming_distances) > 0 else 0
                )
            else:
    ...
    def check_batch(self, fingerprints_batch: np.ndarray) -> DriftMetrics:
        ...
        if len(fingerprints_batch) < self.min_batch_size:
            logger.warning(
                f"Batch size {len(fingerprints_batch)} below minimum {self.min_batch_size}"
            )
        ...
        if self.baseline_stats is not None:
            drift_metrics = self._detect_drift(
                batch_stats, timestamp, len(fingerprints_batch)
            )
        else:
        ...
        return {
            ...
            "batch_stats": {
                "activation_rates": drift_metrics.bit_activation_rates.tolist()
                if isinstance(drift_metrics.bit_activation_rates, np.ndarray)
                else drift_metrics.bit_activation_rates,
    ...
    def _compute_batch_stats(self, fingerprints: np.ndarray) -> Dict:
        ...
        if len(fingerprints) > 1:
            if TEJAS_FAST_HAMMING:
                # Use optimized pairwise distance computation
                n_sample = min(
                    len(fingerprints), 500
                )  # Can handle more with fast method
                sample_idx = np.random.choice(
                    len(fingerprints), n_sample, replace=False
                )
                fp_sample = fingerprints[sample_idx]

                # Compute full pairwise distance matrix for sample
                dist_matrix = fast_pairwise_distances(fp_sample, metric="hamming")
                # Extract upper triangle (excluding diagonal)
                upper_tri_indices = np.triu_indices(n_sample, k=1)
                hamming_distances = (
                    dist_matrix[upper_tri_indices] * fingerprints.shape[1]
                )  # Convert back to counts
    ...
    def _detect_drift(
        self, batch_stats: Dict, timestamp: float, batch_size: int
    ) -> DriftMetrics:
        ...
        try:
            ...
            baseline_hist, _ = np.histogram(
                baseline["bit_activation_rates"], bins=20, range=(0, 1)
            )
            batch_hist, _ = np.histogram(
                batch_stats["activation_rates"], bins=20, range=(0, 1)
            )
            ...
            js_div = (
                jensenshannon(baseline_hist, batch_hist) ** 2
            )  # Square for JS divergence
        ...
        if HAS_SCIPY and stats is not None:
            try:
                ks_stat, ks_pval = stats.ks_2samp(
                    baseline["bit_activation_rates"], batch_stats["activation_rates"]
                )
            except Exception as e:
        ...
        entropy_change = abs(
            batch_stats["activation_entropy"] - baseline["activation_entropy"]
        )
        ...
        hamming_change = abs(
            batch_stats["mean_hamming_distance"] - baseline["mean_hamming_distance"]
        )
        ...
        recommend_recal = (
            ...
            or js_div > self.config["recal_threshold"]
            or ks_pval < self.drift_threshold / 2
        )
    ...
    def get_summary(self, window_size: int = 10) -> Dict:
        ...
        drift_counts = {
            ...
            "moderate": sum(
                1 for m in recent_history if m.drift_severity == "moderate"
            ),
        ...
        return {
            ...
            "drift_rate": sum(drift_counts[k] for k in ["low", "moderate", "severe"])
            / len(recent_history),
    ...
    def recommend_recalibration(self) -> Tuple[bool, str]:
        ...
        high_severity_count = sum(
            1 for m in recent_metrics if m.drift_severity == "severe"
        )

        # Count recalibration recommendations
        recal_recommendations = sum(
            1 for m in recent_metrics if m.recommend_recalibration
        )
        ...
        if high_severity_count >= 3:
            return (
                True,
                f"Multiple high-severity drifts detected ({high_severity_count}/10)",
            )

        if recal_recommendations >= 5:
            return (
                True,
                f"Frequent recalibration recommendations ({recal_recommendations}/10)",
            )
        ...
        if latest.drift_severity == "severe" and latest.confidence_score > 0.8:
            return (
                True,
                f"High-confidence drift detected (confidence: {latest.confidence_score:.2f})",
            )
...
if __name__ == "__main__":
    ...
    monitor = DriftMonitor(
        baseline_fingerprints=baseline_fp, sensitivity="medium", drift_threshold=0.05
    )
